...
if cfg.enable_imselect then
  autocmd("InsertLeave", {
    group = myAutoGroup,
    callback = require("easier.utils.im-select").insertLeave,
  })

  autocmd("InsertEnter", {
    group = myAutoGroup,
    callback = require("easier.utils.im-select").insertEnter,
  })
end
...
autocmd("BufWritePre", {
  group = myAutoGroup,
  pattern = require("easier.env").getFormatOnSavePattern(),
  callback = function()
    vim.lsp.buf.format()
  end,
})
