...
local cfg = pRequire("easier").config.cmp

local pathUtils = require("easier.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "easier", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "easier", "cmp", "snippets", "vscode"),
})
